...
class MiddleRing(QWidget):
    ...
    def __init__(self, master, mosaicClient, controller, light_instance_ref, logger, main_app_instance=None):
        ...
        self.light_group_name = "Middle Ring [Inside]"
        self.name = "Middle Ring [Inside]"
        ...
        initial_brightness = self.light_instance_ref.saved_slider_values.get("Middle Ring Inside Brightness", 3)
        initial_color_code = self.light_instance_ref.saved_slider_values.get("Middle Ring Inside Color Code", "W")
        initial_color_name = self.light_instance_ref.saved_slider_values.get("Middle Ring Inside Color Name", "White")
        ...
        self.colors = ["Red", "Orange", "Yellow", "Green", "Blue", "#4B0082", "#8A2BE2", "White"]
        ...
        self.light_instance_ref.register_slider("Middle Ring [Inside]", self.slider)
    ...
    def _get_button_stylesheet(self, color_hex, is_selected=False):
        ...
        """
        if is_selected: return base_style + selected_style_addition
        return base_style
    ...
    def _handle_slider_release(self, value):
        ...
        if hasattr(self.controller, 'interior_intensity_changed'):
            self.controller.interior_intensity_changed.emit()
    ...
    def _update_and_send_udp(self, slider_value):
        ...
        try:
            ...
            if 0 <= intensityIndex < len(self.matrix[colorIndex]):
                ...
                self.logger.log(f"Sending UDP from {self.name}: LZ8{resultCombo}")
                self.mosaicClient.send_message(f"LZ8{resultCombo}", [])
            else:
                print(f"Invalid intensity index: {intensityIndex}. Cannot send message.")
        except Exception as e:
            print(f"NETWORK ERROR in MiddleRing (Inside) page: Could not send UDP message. {e}")
    ...
    def adjust_intensity_by_delta(self, delta, synced):
        if self.is_locked_at_zero: return
    ...
    def update_color_and_intensity(self, light_group_name: str, color_code: str, intensity: int):
        if light_group_name == self.light_group_name:
            ...
            try: self.lastSelectedColor = self.colorNames[self.code.index(color_code)]
            except ValueError: self.lastSelectedColor = "White"
