...
class TopRing(QWidget):
    ...
    def __init__(self, master, mosaicClient, controller, light_instance_ref, logger, main_app_instance=None):
        ...
        self.light_group_name = "Top Ring [Inside]"
        self.name = "Top Ring [Inside]"
        ...
        initial_brightness = self.light_instance_ref.saved_slider_values.get("Top Ring Inside Brightness", 3)
        initial_color_code = self.light_instance_ref.saved_slider_values.get("Top Ring Inside Color Code", "W")
        initial_color_name = self.light_instance_ref.saved_slider_values.get("Top Ring Inside Color Name", "White")
        ...
        self.light_instance_ref.register_slider("Top Ring [Inside]", self.slider)
    ...
    def _update_and_send_udp(self, slider_value):
        ...
        try:
            ...
            if 0 <= intensityIndex < len(self.matrix[colorIndex]):
                ...
                self.logger.log(f"Sending UDP from {self.name}: LZ7{resultCombo}")
                self.mosaicClient.send_message(f"LZ7{resultCombo}", [])
            else:
                print(f"Invalid intensity index: {intensityIndex}. Cannot send message.")
        except Exception as e:
            print(f"NETWORK ERROR in TopRing (Inside) page: Could not send UDP message. {e}")
