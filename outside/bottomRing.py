...
class BottomRing(QWidget):
    ...
    def __init__(self, master, mosaicClient, controller, light_instance_ref, logger, main_app_instance=None):
        ...
        self.light_group_name = "Bottom Ring [Outside]"
        self.name = "Bottom Ring"
        ...
        initial_brightness = self.light_instance_ref.saved_slider_values.get("Bottom Ring Outside Brightness", 3)
        initial_color_code = self.light_instance_ref.saved_slider_values.get("Bottom Ring Outside Color Code", "W")
        initial_color_name = self.light_instance_ref.saved_slider_values.get("Bottom Ring Outside Color Name", "White")
        ...
        self.light_instance_ref.register_slider("Bottom Ring [Outside]", self.slider)
    ...
    def _update_and_send_udp(self, slider_value):
        ...
        try:
            ...
            if 0 <= intensityIndex < len(self.matrix[colorIndex]):
                ...
                self.logger.log(f"Sending UDP from {self.name}: LZ3{resultCombo}")
                self.mosaicClient.send_message(f"LZ3{resultCombo}", [])
            else:
                print(f"Invalid intensity index: {intensityIndex}. Cannot send message.")
        except Exception as e:
            print(f"NETWORK ERROR in BottomRing page: Could not send UDP message. {e}")
    ...
    def adjust_intensity_by_delta(self, delta, synced):
        ...
        if new_intensity != current_intensity: self.set_intensity(new_intensity)
