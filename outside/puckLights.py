...
class PuckLights(QWidget):
    ...
    def __init__(self, master, mosaicClient, controller, light_instance_ref, logger, main_app_instance=None):
        ...
        self.light_group_name = "Puck Lights"
        self.name = "Puck Lights"
        ...
        initial_brightness = self.light_instance_ref.saved_slider_values.get("Puck Lights Brightness", 3)
        initial_color_code = self.light_instance_ref.saved_slider_values.get("Puck Lights Color Code", "W")
        initial_color_name = self.light_instance_ref.saved_slider_values.get("Puck Lights Color Name", "White")
        ...
        self.light_instance_ref.register_slider("Puck Lights", self.slider)
    ...
    def _update_and_send_udp(self, slider_value):
        ...
        try:
            ...
            if 0 <= intensityIndex < len(self.matrix[colorIndex]):
                ...
                self.logger.log(f"Sending UDP from {self.name}: LZ4{resultCombo}")
                self.mosaicClient.send_message(f"LZ4{resultCombo}", [])
            else:
                print(f"Invalid intensity index: {intensityIndex}. Cannot send message.")
        except Exception as e:
            print(f"NETWORK ERROR in PuckLights page: Could not send UDP message. {e}")
