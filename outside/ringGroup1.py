...
class RingGroup1(QWidget):
    ...
    def __init__(self, master, mosaicClient, controller, light_instance_ref, logger, main_app_instance=None):
        ...
        self.light_group_name = "Ring Group 1"
        self.name = "Ring Group 1"
        ...
        initial_brightness = self.light_instance_ref.saved_slider_values.get("Ring Group 1 Brightness", 3)
        initial_color_code = self.light_instance_ref.saved_slider_values.get("Ring Group 1 Color Code", "W")
        initial_color_name = self.light_instance_ref.saved_slider_values.get("Ring Group 1 Color Name", "White")
        ...
        page_label = QLabel("Ring Group 1")
        ...
        self.light_instance_ref.register_slider("Ring Group 1", self.slider)
    ...
    def _update_and_send_udp(self, slider_value):
        ...
        try:
            ...
            if 0 <= intensityIndex < len(self.matrix[colorIndex]):
                ...
                self.logger.log(f"Sending UDP from {self.name}: LZ1{resultCombo}")
                self.mosaicClient.send_message(f"LZ1{resultCombo}", [])
            else:
                print(f"Invalid intensity index: {intensityIndex}. Cannot send message.")
        except Exception as e:
            print(f"NETWORK ERROR in RingGroup1 page: Could not send UDP message. {e}")
